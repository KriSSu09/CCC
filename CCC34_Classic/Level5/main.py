...
def find_lowest_price(prices, task):
    for price in prices:
        if price['concurrent_tasks'] < maxConcurrentTasks:
...
def add_in_power_draw(task, given_minute):
    ...
    given_minute['concurrent_tasks'] += 1


f = open("level5_1.in", "r")
g = open("output.txt", "w")
maxPower = int(f.readline())
maxElectricityBill = int(f.readline())
maxConcurrentTasks = int(f.readline())
N = int(f.readline())
prices = []
for i in range(N):
    temp_price = {'minute': i, 'price': int(f.readline()), 'power_draw': 0, 'concurrent_tasks': 0}
    prices.append(temp_price)
tasks = []
M = int(f.readline())
for i in range(M):
    input_string = f.readline().split()
    task = {'id': int(input_string[0]), 'power': int(input_string[1]), 'start_interval': int(input_string[2]),
            'end_interval': int(input_string[3]), 'power_draw': [], 'min_cost': 999999999999999999999999999999}
    tasks.append(task)
...
current_bill = 0
# for price in prices:
#     while price['power_draw'] < maxPower and price['concurrent_tasks'] < maxConcurrentTasks:
#         current_task = find_lowest_task(tasks, price)
#         if current_task is None:
#             break
#         draw_time = 0
#         while current_task['power'] != 0 and price['power_draw'] < maxPower and price['concurrent_tasks'] < maxConcurrentTasks:
#             draw_time += 1
#             current_task['power'] -= 1
#             price['power_draw'] += 1
#             price['concurrent_tasks'] += 1
